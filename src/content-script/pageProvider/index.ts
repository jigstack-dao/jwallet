...
const ele = document.getElementById('jwallet');
const channelName = ele?.getAttribute('class') || undefined;

const log = (event, ...args) => {
  if (process.env.NODE_ENV !== 'production') {
    console.log(
      `%c [Jigstack] (${new Date().toTimeString().substr(0, 8)}) ${event}`,
  ...
  _request = async (data) => {
    ...
    return this._requestPromise.call(() => {
      ...
      return this._bcm
        ...
        .then((res) => {
          if (method !== 'eth_call') {
            log('[request: success]', method, res);
          }
          return res;
        })
        .catch((err) => {
          if (method !== 'eth_call') {
            log('[request: error]', method, serializeError(err));
          }
