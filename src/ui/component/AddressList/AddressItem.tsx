...
export interface AddressItemProps {
  ...
  className?: string;
  hiddenAddresses?: Array<{ type: string; address: string }>;
      ...
      const changeName = async () => {
        if (!alianName) {
          ...
          if (existAlianName) {
            ...
            const _accounts = await wallet.getAllVisibleAccountsArray();

            const index = _accounts.filter(
              (acc) => acc.type === 'WalletConnect'
            ).length;
            const alianName = `${
              BRAND_ALIAN_TYPE_TEXT[account?.brandName || account?.type] ||
              account?.brandName
            } ${importedLength + index}`;
              ...
              {(showImportIcon || editing) && (
                <div className="brand-name flex">
                  {!stopEditing && editing ? (
                    ...
                    <div className="display-name text-white text-[16px] ">
                      {displayName}
                    </div>
                  )}
                  {stopEditing && editing && (
                ...
                <AddressViewer
                  ...
                  className={
                    showImportIcon || !showIndex
                      ? 'text-[14px] subtitle text-white'
                      : 'text-[14px] import-color flex  text-white'
                  }
                  showIndex={showIndex}
                />
                <Copy data={account?.address} className="w-4 h-4 ml-4"></Copy>
              </div>
            </div>
          </div>
          {keyring && (
