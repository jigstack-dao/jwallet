...
class PreferenceService {
  ...
  init = async () => {
    ...
    if (!this.store.walletLockTime) {
      this.store.walletLockTime = 0;
    }
  };
  ...
  setPopupOpen = (isOpen) => {
    if (this.store.walletLockTime < new Date().getTime() && isOpen) {
  ...
  updateWalletLockTime = (mins: number) => {
    this.store.walletLockTime = new Date(
      new Date().getTime() + mins * 60000
    ).getTime();
  };
}
