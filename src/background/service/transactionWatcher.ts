...
class TransactionWatcher {
  ...
  addTx = (
    id: string,
    { hash, chainId, nonce }: { hash: string; chainId: number; nonce: string }
  ) => {
    this.store.pendingTx = {
      ...this.store.pendingTx,
      [id]: new Transaction(nonce, hash, chainId),
    };
    const scanLink = permissionService.getNetworkByChainId(chainId).scanLink;
    const url = format(scanLink, hash);
    notification.create(
      url,
  ...
  checkStatus = async (id: string) => {
    ...
    const { hash } = this.store.pendingTx[id];
  ...
  notify = (id: string, txReceipt) => {
    ...
    const { hash, chainId, nonce } = this.store.pendingTx[id];
    const scanLink = permissionService.getNetworkByChainId(chainId).scanLink;
    const url = format(scanLink, hash);
