...
import { ethers, Contract, BigNumberish, providers, BigNumber } from 'ethers';
...
import { ConnectedSite, NetworkChain, TokenSaved } from '../service/permission';
...
import { AdvancedSetting, GeneralSetting } from '../service/setting';
...
import SantaAPI from '../service/santaAPI';

const stashKeyrings: Record<string, any> = {};
let walletClosingTime = new Date(new Date().getTime() + 3 * 60000);
...
export class WalletController extends BaseController {
  ...
  Swap = async (route: Route, gasPrice?: BigNumberish) => {
    const networks = this.getAllNetworks();
    const fromNetwork = networks.find(
      (network) => +network.chainId === +route.fromChainId
    );
    const provider = new ethers.providers.JsonRpcProvider(fromNetwork?.rpcURL);

    const stepPromises = route.steps.map((step) => {
      let allowance: BigNumber | Promise<BigNumber>;
      if (step.action.fromToken.address === ethers.constants.AddressZero) {
        ...
        allowance = erc20Service.getAllowance(
          route.fromAddress || ethers.constants.AddressZero,
          lifiAddress
        );
      }

      return Promise.all([lifiAPI.getAdvancedStep(step), allowance]);
    });

    type RequestWithPayload = providers.TransactionRequest & AdditionalPayload;

    const txSteps: RequestWithPayload[] = [];
    const stepResponses = await Promise.all(stepPromises);
    stepResponses.forEach((res) => {
      const [step, allowance] = res;
      if (allowance.lt(step.action.fromAmount)) {
        ...
        txSteps.push({
          ...
          data: erc20Service.encodeApprove(
            lifiAddress,
            ethers.constants.MaxUint256._hex
          ),
          gasPrice,
      ...
      if (step.transactionRequest) {
        txSteps.push({
          ...
          amount: BigNumber.from(step.action.fromAmount),
          amountTo: BigNumber.from(step.estimate.toAmount),
          token: {
            ...step.action.fromToken,
            isNative: step.action.fromToken.address === AddressZero,
          },
          tokenTo: {
            ...step.action.toToken,
            isNative: step.action.toToken.address === AddressZero,
          },
        });
      }
    });
    for (let i = 0; i < txSteps.length; i++) {
      ...
      const payload = {
        ...
        amount: step.amount,
        amountTo: step.amountTo,
        token: step.token,
        tokenTo: step.tokenTo,
      };

      delete step.transactionType;
      delete step.amount;
      delete step.amountTo;
      delete step.token;
      delete step.tokenTo;

      if (step) {
        transactionHistoryService.cacheAdditionalPayload(payload);
        await this.sendRequest({
          method: 'eth_sendTransaction',
          params: [step],
        });
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (transaction) {
          ...
          const santaAPI = new SantaAPI(token);
          santaAPI.SendGift({ ...apiBody, txhash });
  ...
  getCurrentSite = (tabId: number): ConnectedSite | null => {
    const { name, icon, origin } = sessionService.getSession(tabId) || {};
    if (!origin) {
      return null;
    }
  ...
  setSite = (data: ConnectedSite) => {
    ...
    if (data.isConnected) {
      sessionService.broadcastEvent(
        'chainChanged',
        {
          chain: numberToHex(data.chainId),
          networkVersion: data.chainId.toString(),
        },
        data.origin
      );
    }
  ...
  updateConnectSite = (origin: string, data: ConnectedSite) => {
    ...
    sessionService.broadcastEvent(
      'chainChanged',
      {
        chain: intToHex(data.chainId),
        networkVersion: String(data.chainId),
      },
      data.origin
    );
  };
  ...
  importWatchAddress = async (address) => {
    ...
    await this.updateAlianNameWithKeyring(address, keyringType);
    return this._setCurrentAccountFromKeyring(keyring, -1);
  };
    ...
    try {
      ...
      keyring = new WalletConnect({
        ...
        clientMeta: {
          ...
          url: 'https://jigstack.org/',
          icons: ['https://jigstack.org/static/media/logo_white.4bfe4e3b.svg'],
          name: 'Jigstack',
        },
      });
      isNewKey = true;
    }
    const connector = await keyring.initConnector(brandName, bridge);
    const { uri } = connector;
  ...
  getWalletConnectConnectors = () => {
    ...
    const keyring: WalletConnectKeyring = this._getKeyringByType(keyringType);
    if (keyring) {
      const result: Array<{ address: string; brandName: string }> = [];
  ...
  sendRequestToWalletConnect = async (
    address: string,
    {
      params,
      method,
    ...
    const account = keyring.accounts.find(
      (acct) => acct.address.toLowerCase() === address.toLowerCase()
    );
    if (!account) {
      throw new Error('Can not find this address');
    }
    keyring.onAfterConnect = async (error?, payload?) => {
      if (error) {
        keyring.updateCurrentStatus(
      ...
      if (!keyring.currentConnector) throw new Error('No connector avaliable');
      ...
      try {
        ...
        const result =
          await keyring.currentConnector.connector.sendCustomRequest({
            method,
            params,
          });
        keyring.resolvePromise!(result);
        keyring.updateCurrentStatus(
          WALLETCONNECT_STATUS_MAP.SUBMITTED,
          account,
          result
        );
      } catch (e) {
    ...
    keyring.onDisconnect = (error, payload) => {
      if (!keyring.currentConnector) throw new Error('No connector avaliable');
      ...
      keyring.closeConnector(
        keyring.currentConnector.connector,
        address,
        account.brandName
      );
    };

    await keyring.init(account.address, account.brandName);

    return new Promise((resolve) => {
      keyring.resolvePromise = resolve;
    });
  };
  ...
  importPrivateKey = async (data) => {
    ...
    const error = new Error(ERROR_MESSAGES.PRIVATE_KEY_IS_INVALID);
    try {
  ...
  importJson = async (content: string, password: string) => {
    try {
      ...
      throw new Error(ERROR_MESSAGES.JSON_FILE_TYPE_INVALID);
    }
  ...
  generateKeyringWithMnemonic = (mnemonic) => {
    if (!bip39.validateMnemonic(mnemonic)) {
      throw new Error(ERROR_MESSAGES.MNEMONIC_IS_INVALID);
    }
  ...
  deriveNewAccountFromMnemonic = async () => {
    let keyring, isNewKey;
    const keyringType = KEYRING_CLASS.MNEMONIC;
    try {
      keyring = this._getKeyringByType(keyringType);
    } catch (error) {
      const MenemonicKeyring =
        keyringService.getKeyringClassForType(keyringType);
      keyring = new MenemonicKeyring();
      isNewKey = true;
    }
  ...
  getPrivateKeyInternal = async (from: string) => {
    return (await keyringService?.exportAccount(from)) || '';
  };
  ...
  cacheAdditionalPayload = (cache: AdditionalPayload) =>
    transactionHistoryService.cacheAdditionalPayload(cache);

  getAdditionalPayload = () => transactionHistoryService.getAdditionalPayload();
  ...
  getPendingNonce = (address: string, chainId: number) =>
    transactionHistoryService.getPendingNonce(address, chainId);

  getMinPendingNonce = (address: string, chainId: number) =>
    transactionHistoryService.getMinPendingNonce(address, chainId);
  ...
  isExportSeed() {
    const keyring = keyringService.getKeyringByType(KEYRING_TYPE.HdKeyring);
    return !!keyring;
  }
  ...
  updateAlianNameWithKeyring = async (address: string, keyringType = '') => {
    const accounts = await this.getAllVisibleAccountsArray();
    const counter = keyringType
      ? accounts.filter((x) => x.type == keyringType).length
      : accounts.length;
    switch (keyringType) {
      case KEYRING_CLASS.WATCH:
        this.updateAlianName(address, `Watch ${counter}`);
        break;

      default:
        this.updateAlianName(address, `Account ${counter}`);
        break;
    }
  };

  setCurrentNetworkTemporary = (chainId: number) =>
    permissionService.setCurrentNetworkTemporary(chainId);

  getCurrentNetworkTemporary = () =>
    permissionService.getCurrentNetworkTemporary();

  getAllNetworks = () => permissionService.getAllNetworks();

  getNetworkByChainId = (chainId: number) =>
    permissionService.getNetworkByChainId(chainId);

  upsertNetwork = (network: NetworkChain, oldChain = 0) => {
    permissionService.upsertNetwork(network, oldChain);
  };

  setClosingTime = () => {
    const fiveMinutes = 5 * 60000;
    walletClosingTime = new Date(new Date().getTime() + fiveMinutes);
  };

  isExpiredClosingTime = () => {
    return new Date().getTime() > walletClosingTime.getTime();
  };

  getOriginalAddress = async (address: string, type: string) => {
    return type !== 'WalletConnect'
      ? new ethers.Wallet(await this.getPrivateKeyInternal(address)).address
      : address;
  };

  getAllTokens = () => permissionService.getAllTokens();

  getTokensByChainId = (chainId: number) =>
    permissionService.getTokensByChainId(chainId);

  importTokens = (tokens: TokenSaved[]) => {
    permissionService.importTokens(tokens);
  };
  ...
  addTransactionPending(tx: TransactionPendingItem) {
    transactionHistoryService.addTransactionPending(tx);
  }

  addTransactionCompleted(tx: TransactionsCompletedItem) {
    transactionHistoryService.addTransactionCompleted(tx);
  }

  getTransactionPending(address: string) {
    return transactionHistoryService.getTransactionPending(address);
  }

  getTransactionsCompleted(address: string) {
    return transactionHistoryService.getTransactionsCompleted(address);
  }

  getGeneralSetting() {
    return settingService.getStore().general;
  }

  getAdvancedSetting() {
    return settingService.getStore().advanced;
  }

  updateGeneralSetting(general: GeneralSetting) {
    settingService.updateGeneraSetting(general);
  }

  updateAdvancedSetting(advanced: AdvancedSetting) {
    settingService.updateAdvancedSetting(advanced);
  }
}
